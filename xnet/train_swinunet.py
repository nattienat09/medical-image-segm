...
def train(epochs, batch_size, model_save_dir):
    ...
    for e in range(1, epochs+1):
        ...
        if(res > max_val_dice):
            max_val_dice = res
            G.save(model_save_dir + 'swinunet_cvc_augm_retry')
            G.save_weights(model_save_dir + 'swinunet_cvc_augm_retry_weights')
            print('New Val_Dice HighScore',res)            
            
model_save_dir = '/unet/swinunet_cvc_augm_retry_model/'
