...
def train(epochs, batch_size, model_save_dir):
    ...
    max_val_dice= -1
    G = DeeplabV3Plus(image_size=SIZE_1, num_classes=2)
    ...
    for e in range(1, epochs+1):
        ...
        Y_val_2 = (Y_val[:,:,:,1] >=0.5).astype(int)
        res = mean_dice_coef(Y_val_2,y_pred)
        if(res > max_val_dice):
            max_val_dice = res
            G.save(model_save_dir + 'deeplab_polyps_augm')
            G.save_weights(model_save_dir + 'deeplab_polyps_augm_weights')
            print('New Val_Dice HighScore',res)            
            
model_save_dir = '/deeplabv3+/deeplab_polyps_augm_model/'
