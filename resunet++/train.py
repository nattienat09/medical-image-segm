...
def train(epochs, batch_size, model_save_dir):
    ...
    max_val_dice= -1
    arch = ResUnetPlusPlus(input_size=(SIZE_1,SIZE_2,3))
    G = arch.build_model()
    ...
    for e in range(1, epochs+1):
        ...
        if(res > max_val_dice):
            max_val_dice = res
            G.save(model_save_dir + 'resunetpp_cells_noaugm_retry')
            G.save_weights(model_save_dir + 'resunetpp_cells_noaugm_retry_weights')
            print('New Val_Dice HighScore',res)            
            
model_save_dir = '/resunetplusplus/resunetpp_cells_noaugm_retry_model/'
